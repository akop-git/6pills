          ...
          <section key={pillarId} style={{marginBottom:44}}>
            ...
            <div style={{display:'flex',flexDirection:'column'}}>
              {stems.map(s => {
                ...
                return (
                  <button key={s.id} onClick={() => count > 0 ?fontSize:36const coy={pillarId} sP || u if (i === brary',lpractcolumn'}}>
              {stems.map(s => {
            }13,ce   wro   k...', pillar: 'inte
  os>{pil2', deser d8,paddingBc0')}</div>
:13olid ${pillar.color}55`}}>
      )iS const en/e={{textAlign:'center',marginTop:12,color:COLORS.inkFaint}}>Write fast. Don't edit. There are no wrong answers.</div>
      </div>
    </bntSizast. Don'eek = Don'inTop:12,color:COLORS.inkFa= Don'inTop:12e>
  h COLORS.inkFa= Donr:COLORS.in0 TherFa= DonpillarId} nswe>All senteT' 30; }
        .stem-i="f !viewSdone?pillar   kStemyle=h.stem-i="f !viewS="f !viewS=msnswers.</div>
  OLORS.innPick.te
u,t',winck.t== py=.01em'}}>{pitings: 'opsz' 144, 'SOFs6"  </b]  5a' 144itings: 'opsz' 144, 'SO]-o={onPractice [lignItems:'center',gap:6}}>
          <Plus size={1,dm]d} sPick.etio.stemYd,:11,minW,dm]d} sPick.etio.stemYd,:11,minW,dm]d} sPick.e}  k..eo.slay:'fled (i === braryEPick.e}  k..eo.slay:'fled (i === braryEPick.e}  k..eo.slay:'f{pilla       <div cb)k: 'idcountk.e} 